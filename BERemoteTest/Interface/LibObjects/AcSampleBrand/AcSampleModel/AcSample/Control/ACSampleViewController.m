...
#import "BIRGUIFeature.h"

@interface ACSampleViewController () <UITableViewDataSource,UITableViewDelegate>
{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([irkey isEqualToString:@"IR_ACKEY_TEMP"]) {
        NSString* temp = [_temperatureArray objectAtIndex:[_remoter getKeyOption:@"IR_ACKEY_TEMP"].currentOption];
        NSRange range = [temp rangeOfString:@"IR_ACSTATE_TEMP_"];
        if (NSNotFound != range.location)
        {
            [_temperatureStepper setValue:[[temp substringFromIndex:range.location+range.length] doubleValue]];
        }
        else
        {
            NSRange range = [self.temperature rangeOfString:@"IR_ACOPT_TEMP_"];
