...
@interface ListViewController () <UISearchResultsUpdating, UISearchBarDelegate> {
...
- (void)viewDidLoad {
    ...
    if ([self.className isEqualToString:@"BIRTypeItem"]) {
        ...
        _listArray = [_dataProvider getModelListWithType:self.typeID andBrand:self.brandID getNew:NO];
    ...
    }else if ([self.className isEqualToString:@"BIRTVPicker"]) {
        _listArray = [_dataProvider getBrandListWithType:@"1"];
    }else if ([self.className isEqualToString:@"BIRRemote"]) {
        _listArray = [_dataProvider getTypeList];
    }else if ([self.className isEqualToString:@"BIRRemoteBrand"]) {
        _listArray = [_dataProvider getBrandListWithType:self.typeID];
    }else if (([self.className isEqualToString:@"BIRRemoteModel"]) ||
              ([self.className isEqualToString:@"TestACModel"]) ){
        _listArray = [_dataProvider getModelListWithType:self.typeID andBrand:self.brandID getNew:NO];
    }else if ([self.className isEqualToString:@"TestAC"]){
        _listArray = [_dataProvider getBrandListWithType:@"2"];
    }
    
    if ([self.className isEqualToString:@"BIRRemoteModel"]){
        UIBarButtonItem *naviLeftButton = [[UIBarButtonItem alloc] initWithTitle:@"小萬能" style:UIBarButtonItemStylePlain target:self action:@selector(createUniversalRemote)];
        self.navigationItem.rightBarButtonItem = naviLeftButton;
    }
    if ([self.className isEqualToString:@"TestACModel"]){
        UIBarButtonItem *naviLeftButton = [[UIBarButtonItem alloc] initWithTitle:@"小萬能" style:UIBarButtonItemStylePlain target:self action:@selector(createACUniversalRemote)];
        self.navigationItem.rightBarButtonItem = naviLeftButton;
    }
}
...
-(void) createACUniversalRemote{
    ACRemoteViewController* view = [[ACRemoteViewController alloc] initWithNibName:@"ACRemoteViewController" bundle:nil];
    view.typeID = self.typeID;
    view.brandID = self.brandID;
    view.modelID = nil;
    view.brandName = @"小萬能";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (([self.className isEqualToString:@"BIRTypeItem"]) ||
       ([self.className isEqualToString:@"BIRRemote"])) {
        ...
        cellText = [NSString stringWithFormat:@"%@,%@",item.typeId,item.name];
              ...
              ([self.className isEqualToString:@"BIRRemoteBrand"]) ||
              ([self.className isEqualToString:@"TestAC"]) ) {
        BIRBrandItem *item = [tempArray objectAtIndex:indexPath.row];
        cellText = item.name;
    }else if (([self.className isEqualToString:@"BIRModelItem"]) ||
              ([self.className isEqualToString:@"BIRRemoteModel"]) ||
              ([self.className isEqualToString:@"TestACModel"])) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.className isEqualToString:@"BIRTVPicker"]) {
        ...
        ListViewController* view = [[ListViewController alloc] initWithNibName:@"ListViewController" bundle:nil];
        BIRTypeItem *item = [tempArray objectAtIndex:indexPath.row];
        view.className = [self.className stringByAppendingString:@"Brand"];
        view.typeID = item.typeId;
        [self.navigationController pushViewController:view animated:YES];
    }else if (([self.className isEqualToString:@"BIRRemoteBrand"]) ||
              ([self.className isEqualToString:@"TestAC"]) ){
        ListViewController* view = [[ListViewController alloc] initWithNibName:@"ListViewController" bundle:nil];
        BIRBrandItem *item = [tempArray objectAtIndex:indexPath.row];
        NSString* name;
        if ([self.className isEqualToString:@"BIRRemoteBrand"]) {
            name = @"BIRRemoteModel";
        }else{
            name = @"TestACModel";
        }
        ...
        view.typeID = self.typeID;
        view.brandID = self.brandID;
        view.modelID = item.model;
        view.brandName = item.model;
        [self.navigationController pushViewController:view animated:YES];
    }else if ([self.className isEqualToString:@"TestACModel"]){
        ACRemoteViewController* view = [[ACRemoteViewController alloc] initWithNibName:@"ACRemoteViewController" bundle:nil];
        BIRModelItem *item = [tempArray objectAtIndex:indexPath.row];
        view.typeID = self.typeID;
        view.brandID = self.brandID;
